...
.input-number {
  width: 60px; /* Menyesuaikan lebar input */
  text-align: center;
  padding: 5px;
  margin: 0;
  border: 1px solid #ccc; /* Border di input */
}
...
.btn-number:hover {
  background-color: #f1f1f1; /* Efek hover untuk tombol */
}
...
<table class="table">
    <thead>
      <tr>
        ...
        <th style="text-align: center; padding-right: 19px;">Quantity</th>
        <th>Unit Price</th>
        <th>Type</th>
        <th>Delete</th>
        <th>Sub Total</th>
      ...
      <tr>
        ...
        <td>
          <div class="input-group">
            <button type="button" class="btn btn-default btn-number" data-type="minus" data-field="quantity-<?php echo $pro_id; ?>">-</button>
            <input type="text" name="quantity-<?php echo $pro_id; ?>" class="form-control quantity input-number" value="<?php echo $_SESSION['pro_qty']; ?>" data-product_id="<?php echo $pro_id; ?>" style="width: 60px; text-align: center;">
            <button type="button" class="btn btn-default btn-number" data-type="plus" data-field="quantity-<?php echo $pro_id; ?>">+</button>
          </div>
        </td>
        <td>Rp<?php echo number_format($only_price, 0, ',', '.'); ?></td>
...
<div class='col-md-3 col-sm-6 center-responsive'>
    <div class='product'>
        ...
        <div class='text'>
            ...
            <p class='price'>
                Rp " . number_format((float)str_replace(['Rp', '.', ','], '', $product_price), 0, ',', '.') . " 
                " . ($product_psp_price ? "| Rp " . number_format((float)str_replace(['Rp', '.', ','], '', $product_psp_price), 0, ',', '.') : "") . "
            </p>
...
<th style="white-space: nowrap;">Rp&nbsp;<?php echo number_format($total, 0, ',', '.'); ?></th>
...
// Fungsi untuk mengirim data ke server ketika kuantitas diubah
function updateCartQuantity(input) {
  var id = input.data("product_id");
  var quantity = input.val();

  if (quantity != '') {
    $.ajax({
      url: "functions/change.php",
      method: "POST",
      data: {id: id, quantity: quantity},
      success: function(data) {
        $("body").load('functions/cart_body.php'); // Perbarui keranjang setelah perubahan
      }
    });
  }
}
